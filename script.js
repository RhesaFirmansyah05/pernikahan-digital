...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 11. LISTEN FOR STORAGE CHANGES FROM ADMIN TAB
  window.addEventListener('storage', (e) => {
    if (e.key === 'wedding_config') {
      currentConfig = getWeddingConfig();
      applyConfigToDOM(currentConfig);
    }
  });
...
function applyConfigToDOM(cfg) {
  ...
  const timelineContainerEl = document.querySelector('.timeline');
  if (timelineContainerEl && cfg.timelineItems && Array.isArray(cfg.timelineItems)) {
    timelineContainerEl.innerHTML = '';
    cfg.timelineItems.forEach((story) => {
      const item = document.createElement('div');
      item.className = 'timeline-item reveal active';
      item.innerHTML = `
        <div class="timeline-dot"></div>
        <div class="timeline-content card-glass">
          <span class="timeline-date">${escapeHtml(story.date)}</span>
          <h3 class="timeline-title">${escapeHtml(story.title)}</h3>
          <p class="timeline-desc">${escapeHtml(story.desc)}</p>
        </div>
      `;
...
/* ==========================================================================
   1. GUEST NAME FROM URL (?to=Nama%20Tamu) & REGENERATE ENGINE
   ========================================================================== */
function initGuestName() {
  const urlParams = new URLSearchParams(window.location.search);
  const rawGuest = urlParams.get('to') || urlParams.get('nama') || urlParams.get('guest');
  const isRegenerateRequested = urlParams.has('regenerate') || window.location.hash === '#regenerate';

  const guestDisplayEl = document.getElementById('guest-name');
  const wishNameInput = document.getElementById('nama-tamu');
  const btnToggleEdit = document.getElementById('btn-toggle-edit-guest');
  const editBox = document.getElementById('guest-edit-box');
  const inputCustomGuest = document.getElementById('input-custom-guest');
  const btnApplyGuest = document.getElementById('btn-apply-guest');
  const btnCopyGuestLink = document.getElementById('btn-copy-guest-link');

  let currentGuestName = 'Bapak/Ibu/Saudara/i';

  if (rawGuest && rawGuest.trim() !== '') {
    currentGuestName = decodeURIComponent(rawGuest.replace(/\+/g, ' ')).trim();
  }

  const updateAllGuestNameReferences = (newName) => {
    const finalName = newName.trim() || 'Bapak/Ibu/Saudara/i';
    currentGuestName = finalName;

    if (guestDisplayEl) {
      guestDisplayEl.textContent = finalName;
    }
    if (wishNameInput && finalName !== 'Bapak/Ibu/Saudara/i') {
    ...
    if (window.history && window.history.replaceState) {
      ...
      if (finalName !== 'Bapak/Ibu/Saudara/i' && finalName !== '') {
        newUrl.searchParams.set('to', finalName);
      } else {
  ...
  // Initial set
  updateAllGuestNameReferences(currentGuestName);
  if (inputCustomGuest && currentGuestName !== 'Bapak/Ibu/Saudara/i') {
    inputCustomGuest.value = currentGuestName;
  }

  const actionWrapper = document.getElementById('guest-action-wrapper');

  // Auto expand edit box ONLY if explicitly requested via URL (?regenerate=true or #regenerate)
  if (isRegenerateRequested) {
    if (actionWrapper) actionWrapper.style.display = 'block';
  ...
  // Toggle edit box display
  if (btnToggleEdit && editBox) {
    btnToggleEdit.addEventListener('click', () => {
      const isHidden = editBox.style.display === 'none' || editBox.style.display === '';
      editBox.style.display = isHidden ? 'block' : 'none';
      if (isHidden && inputCustomGuest) {
        inputCustomGuest.focus();
      }
    });
  }

  // Real-time & apply button update
  if (inputCustomGuest) {
    inputCustomGuest.addEventListener('input', (e) => {
      updateAllGuestNameReferences(e.target.value);
    });
  }

  if (btnApplyGuest) {
    btnApplyGuest.addEventListener('click', () => {
      const val = inputCustomGuest ? inputCustomGuest.value : '';
      updateAllGuestNameReferences(val);
      showToast(`Nama tamu diubah menjadi "${currentGuestName}"!`, 'success');
      if (editBox) editBox.style.display = 'none';
    });
  }

  // Copy customized guest link button
  if (btnCopyGuestLink) {
    btnCopyGuestLink.addEventListener('click', () => {
      const nameVal = inputCustomGuest && inputCustomGuest.value.trim() !== '' 
        ? inputCustomGuest.value.trim() 
        : currentGuestName;
      
      const shareUrl = new URL(window.location.origin + window.location.pathname);
      if (nameVal && nameVal !== 'Bapak/Ibu/Saudara/i') {
        shareUrl.searchParams.set('to', nameVal);
      }
      
      copyToClipboard(shareUrl.toString(), () => {
        showToast(`Link undangan untuk "${nameVal}" berhasil disalin!`, 'success');
      });
    });
  }
}
