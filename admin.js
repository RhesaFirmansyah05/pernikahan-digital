...
function saveWeddingConfig(cfg, toastMsg = 'Konfigurasi berhasil disimpan!') {
  try {
    ...
    showToast('Gagal menyimpan: Memori lokal penuh! Hapus beberapa foto atau gunakan URL link foto.', 'error');
  }
}
...
function initPhotosCrud() {
  ...
  if (fileGalInput) {
    fileGalInput.addEventListener('change', (e) => {
      ...
      let processed = 0;
      files.forEach(file => {
        compressImageFile(file, 1000, 0.75, (dataUrl) => {
          currentConfig.galleryPhotos.push(dataUrl);
          processed++;
          if (processed === files.length) {
            saveWeddingConfig(currentConfig, `${files.length} foto berhasil di-upload ke Galeri!`);
            renderGalleryList();
          }
        });
      });
    ...
    fileInput.addEventListener('change', (e) => {
      ...
      if (file) {
        compressImageFile(file, 1000, 0.75, (dataUrl) => {
          ...
          img.src = dataUrl;
          saveAllAdminInputs(true, 'Foto berhasil di-load & disimpan!');
        });
      }
    });
  }
...
function initGuestLinkGenerator() {
  ...
  const btnModeSingle = document.getElementById('btn-mode-single-guest');
  const btnModeBulk = document.getElementById('btn-mode-bulk-guest');
  const boxSingle = document.getElementById('box-single-guest');
  const boxBulk = document.getElementById('box-bulk-guest');

  const bulkTextarea = document.getElementById('gen-guest-bulk-text');
  const bulkResultList = document.getElementById('gen-guest-bulk-result-list');
  const btnBulkSave = document.getElementById('btn-gen-bulk-save');
  const btnBulkCopyAll = document.getElementById('btn-gen-bulk-copy-all');

  // Mode Switcher
  if (btnModeSingle && btnModeBulk && boxSingle && boxBulk) {
    btnModeSingle.addEventListener('click', () => {
      boxSingle.style.display = 'block';
      boxBulk.style.display = 'none';
      btnModeSingle.className = 'btn-gold';
      btnModeBulk.className = 'btn-outline-gold';
    });

    btnModeBulk.addEventListener('click', () => {
      boxSingle.style.display = 'none';
      boxBulk.style.display = 'block';
      btnModeSingle.className = 'btn-outline-gold';
      btnModeBulk.className = 'btn-gold';
      updateBulkList();
    });
  }
  ...
  const btnShareWa = document.getElementById('btn-gen-share-wa');

  if (btnShareWa && nameInput && urlRes) {
    btnShareWa.addEventListener('click', () => {
      const guestName = nameInput.value.trim() || 'Bapak/Ibu/Saudara/i';
      const link = urlRes.value;
      const groom = currentConfig?.groomName || 'Rhesa';
      const bride = currentConfig?.brideName || 'Alya';
      
      const waText = `Kepada Yth. Bapak/Ibu/Saudara/i ${guestName},\n\nTanpa mengurangi rasa hormat, perkenankan kami mengundang Anda untuk menghadiri acara pernikahan kami:\n\n*${groom} & ${bride}*\n\nBerikut link undangan digital Anda:\n${link}\n\nMerupakan suatu kehormatan dan kebahagiaan bagi kami apabila Bapak/Ibu/Saudara/i berkenan hadir dan memberikan doa restu.\n\nTerima kasih.`;
  ...
  function updateBulkList() {
    if (!bulkResultList) return;
    const names = parseBulkNames();
    const baseUrl = window.location.origin + window.location.pathname.replace('admin.html', 'index.html');
    bulkResultList.innerHTML = '';

    if (names.length === 0) {
      bulkResultList.innerHTML = '<p style="color:var(--text-muted); font-size:0.85rem; font-style:italic;">Belum ada nama tamu yang diketik di kolom atas.</p>';
      return;
    }

    names.forEach((name, idx) => {
      const guestUrl = `${baseUrl}?to=${encodeURIComponent(name)}`;
      const itemCard = document.createElement('div');
      itemCard.className = 'crud-card';
      itemCard.style.padding = '10px 14px';
      itemCard.style.display = 'flex';
      itemCard.style.flexDirection = 'column';
      itemCard.style.gap = '6px';

      itemCard.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
          <div style="font-weight: 600; color: var(--gold-light); font-size: 0.95rem;">
          ...
          <div class="action-btn-group" style="flex-wrap: wrap;">
            ...
            <button type="button" class="btn-sm-edit" style="background: rgba(34,197,94,0.2); color: #86EFAC; border-color: rgba(34,197,94,0.5);" onclick="shareGuestHistoryWa('${escapeHtml(name)}', '${escapeHtml(guestUrl)}')">📲 Kirim WA</button>
            <button type="button" class="btn-sm-edit" style="background: rgba(255,255,255,0.1); color: #FFF;" onclick="window.open('${escapeHtml(guestUrl)}', '_blank')">👁️ Uji Buka</button>
          </div>
        </div>
        <div style="font-size: 0.78rem; color: #D6D3D1; word-break: break-all; font-family: monospace; background: rgba(0,0,0,0.3); padding: 4px 8px; border-radius: 6px;">
          ${escapeHtml(guestUrl)}
        </div>
      `;
      bulkResultList.appendChild(itemCard);
    });
  }

  if (bulkTextarea) {
    bulkTextarea.addEventListener('input', updateBulkList);
  }
  ...
  if (btnBulkSave) {
    btnBulkSave.addEventListener('click', () => {
      const names = parseBulkNames();
      if (names.length === 0) {
        showToast('Ketik minimal 1 nama tamu terlebih dahulu.', 'error');
        return;
      }
      ...
      names.forEach(name => {
        // Add if not already top duplicate
        history.unshift({ name, date: today });
        countAdded++;
      });

      saveGuestHistory(history);
      renderGuestHistoryList();
      showToast(`Berhasil menyimpan ${countAdded} nama tamu ke riwayat!`, 'success');
    });
  }

  if (btnBulkCopyAll) {
    btnBulkCopyAll.addEventListener('click', () => {
      const names = parseBulkNames();
      if (names.length === 0) {
        showToast('Ketik minimal 1 nama tamu terlebih dahulu.', 'error');
        return;
      ...
      let fullText = `DAFTAR LINK UNDANGAN UNDANGAN PERNIKAHAN (${groom} & ${bride})\n=========================================\n\n`;

      names.forEach((name, idx) => {
        const guestUrl = `${baseUrl}?to=${encodeURIComponent(name)}`;
        fullText += `${idx + 1}. Kepada Yth. Bapak/Ibu/Saudara/i ${name}\nLink: ${guestUrl}\n\n`;
      });

      navigator.clipboard.writeText(fullText).then(() => {
        showToast(`Berhasil menyalin ${names.length} link & pesan tamu ke clipboard!`, 'success');
      });
    });
  }
}
...
function renderGuestHistoryList() {
  ...
  history.forEach((item, idx) => {
    ...
    card.innerHTML = `
      ...
      <div class="action-btn-group" style="flex-wrap: wrap;">
        <button type="button" class="btn-sm-edit" onclick="copyGuestHistoryUrl('${escapeHtml(guestUrl)}')">📋 Salin Link</button>
        <button type="button" class="btn-sm-edit" style="background: rgba(34,197,94,0.2); color: #86EFAC; border-color: rgba(34,197,94,0.5);" onclick="shareGuestHistoryWa('${escapeHtml(item.name)}', '${escapeHtml(guestUrl)}')">📲 Kirim WA</button>
...
window.shareGuestHistoryWa = function(name, url) {
  const groom = currentConfig?.groomName || 'Rhesa';
  const bride = currentConfig?.brideName || 'Alya';
  const waText = `Kepada Yth. Bapak/Ibu/Saudara/i ${name},\n\nTanpa mengurangi rasa hormat, perkenankan kami mengundang Anda untuk menghadiri acara pernikahan kami:\n\n*${groom} & ${bride}*\n\nBerikut link undangan digital Anda:\n${url}\n\nMerupakan suatu kehormatan dan kebahagiaan bagi kami apabila Bapak/Ibu/Saudara/i berkenan hadir dan memberikan doa restu.\n\nTerima kasih.`;
  const waUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(waText)}`;
  window.open(waUrl, '_blank');
};
...
function saveAllAdminInputs(showToastNotification = true, customToastMsg = null) {
  if (!currentConfig) return;
  
  currentConfig.coverTitle = getValue('#adm-cover-title') || DEFAULT_CONFIG.coverTitle;
  currentConfig.coverDateText = getValue('#adm-cover-date-text') || DEFAULT_CONFIG.coverDateText;
  currentConfig.groomName = getValue('#adm-groom-name') || DEFAULT_CONFIG.groomName;
  currentConfig.groomParents = getValue('#adm-groom-parents') || DEFAULT_CONFIG.groomParents;
  currentConfig.brideName = getValue('#adm-bride-name') || DEFAULT_CONFIG.brideName;
  currentConfig.brideParents = getValue('#adm-bride-parents') || DEFAULT_CONFIG.brideParents;
  currentConfig.quoteText = getValue('#adm-quote-text') || DEFAULT_CONFIG.quoteText;
  currentConfig.quoteSource = getValue('#adm-quote-source') || DEFAULT_CONFIG.quoteSource;

  currentConfig.targetCountdownDate = getValue('#adm-target-countdown') || DEFAULT_CONFIG.targetCountdownDate;

  currentConfig.coverBgUrl = getValue('#adm-cover-bg-url') || DEFAULT_CONFIG.coverBgUrl;
  currentConfig.groomImgUrl = getValue('#adm-groom-img-url') || DEFAULT_CONFIG.groomImgUrl;
  currentConfig.brideImgUrl = getValue('#adm-bride-img-url') || DEFAULT_CONFIG.brideImgUrl;
  currentConfig.coupleImgUrl = getValue('#adm-couple-img-url') || DEFAULT_CONFIG.coupleImgUrl;

  currentConfig.musicUrl = getValue('#adm-music-url') || DEFAULT_CONFIG.musicUrl;

  const newPin = getValue('#adm-new-pin');
  if (newPin) currentConfig.adminPin = newPin;

  saveWeddingConfig(currentConfig, showToastNotification ? (customToastMsg || 'Semua perubahan berhasil disimpan ke website!') : null);
}

function initFormInputs() {
  const inputs = document.querySelectorAll('.admin-tab-pane input, .admin-tab-pane textarea');
  inputs.forEach(input => {
    input.addEventListener('change', () => {
      saveAllAdminInputs(false);
    });
  });
}
...
function initSaveAndReset() {
  ...
  if (btnSaveAll) {
    btnSaveAll.addEventListener('click', () => {
      saveAllAdminInputs(true, 'Semua pengaturan berhasil disimpan ke website!');
    });
  }
...
function initExportCodeTool() {
  const btnExport = document.getElementById('btn-export-code');
  const modal = document.getElementById('modal-export-code');
  const closeBtn = document.getElementById('btn-close-export-modal');
  const closeBtn2 = document.getElementById('btn-close-export');
  ...
  btnExport.addEventListener('click', () => {
    ...
    // Generate formatted JS code string
    const codeStr = `const DEFAULT_CONFIG = ${JSON.stringify(currentConfig, null, 2)};`;
    if (textarea) textarea.value = codeStr;
    modal.classList.add('active');
  });
  ...
  if (copyBtn && textarea) {
    copyBtn.onclick = () => {
      copyToClipboard(textarea.value, () => {
        showToast('Kode DEFAULT_CONFIG berhasil disalin!', 'success');
      });
    };
  }
}
